...
_KEYWORD_TO_DOMAIN: dict[str, str] = {
    ...
    # v1.10.0 M4 fix: word-boundary regex sonrasi alpha-alpha sinirindaki
    # gercek crypto isimlerini korumak icin kombine keyword'ler eklendi.
    "openssl": "crypto", "libssl": "crypto", "libcrypto": "crypto",
    "mbedtls": "crypto", "boringssl": "crypto",
    # Memory
...
# ---------------------------------------------------------------------------
# Pre-compiled regex (module-level) -- performance optimizasyonu
# ---------------------------------------------------------------------------
#
# v1.10.0 Fix Sprint: Her `_classify_domain` cagrisinda ~150 keyword icin
# ayri `re.search` cagrilmasi (her birinde f-string + escape) performans
# darbogaziydi. Tek birlesik alternation pattern ile 100x hizlanma.
#
# Davranis korumasi:
#   - Boundary semantigi: `(?<![a-z]) ... (?![a-z])` aynen korundu.
#   - Iterasyon sirasi: Dict insertion order (Python 3.7+) ile ayni;
#     regex alternation soldan saga dener, bu da ilk-match kuralini
#     korur (orijinal kod da ilk match'te return ediyordu).
#   - Min keyword uzunlugu >= 3 kurali korundu.
#
# NOT: Bu pattern modul seviyesinde bir kez derlenir. Sonraki cagrilarda
# yeniden compile edilmez. `_KEYWORD_TO_DOMAIN` runtime'da degismedigi
# varsayilir (statik tanim).

def _build_keyword_pattern() -> "re.Pattern[str]":
    """_KEYWORD_TO_DOMAIN'den tek birlesik alternation pattern uret."""
    parts = [re.escape(k) for k in _KEYWORD_TO_DOMAIN if len(k) >= 3]
    # (?<![a-z])(kw1|kw2|...)(?![a-z])
    return re.compile(rf"(?<![a-z])({'|'.join(parts)})(?![a-z])")
...
class CalleeProfilePropagator:
    ...
    def __init__(
        ...
        # v1.10.0 Batch 3C Fix #2: Dual accept -- dict veya dataclass
        # (ComputationConfig vb.) kabul et. Dataclass ise asdict ile
        # normalize et; alan ortusmedigi alanlarda varsayilan kullanilir.
        if config is None:
            config = {}
    ...
    def _classify_domain(name: str) -> str:
        ...
        # v1.10.0 M4 fix: naive substring "in" check false positive uretiyor.
        # Yari-word-boundary regex: (?<![a-z]) ... (?![a-z]). 3+ karakter.
        # v1.10.0 Fix Sprint: tek pre-compiled alternation pattern (modul
        # seviyesinde, _KEYWORD_PATTERN). Onceki kod her cagrida 150 ayri
        # re.search yapiyordu; simdi tek cagrida ilk match alinir.
        m = _KEYWORD_PATTERN.search(name_lower)
        if m:
            return _KEYWORD_TO_DOMAIN[m.group(1)]
