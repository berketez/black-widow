...
class ComputationRecoveryEngine:
    ...
    def recover(
        ...
        # --- Layer 3: Signature Fusion (DEPRECATED v1.10.0, removed v1.11.0) ---
        if self._comp_config.enable_signature_fusion:
            warnings.warn(
                "ComputationRecoveryConfig.enable_signature_fusion (Dempster-Shafer "
                "fusion) DEPRECATED; double-counting riski tespit edildi (codex teyit). "
                "v1.11.0'da kaldirilacak. Yerine ComputationConfig."
                "enable_computation_fusion (log-odds ensemble + Platt calibration) "
                "kullanin.",
                DeprecationWarning,
                stacklevel=2,
            )
