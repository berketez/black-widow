"""v1.11.0 Jython Sunset Faz 1.4 (Dalga 6B): export_results.py A/B parity testi.
...
- Bu test suite STATIK parity dogrulamasi yapar:
  ...
  * Fonksiyon/degisken yapisinin (get_output_dir, load_json_safe,
    get_program_info, main) iki dosyada da ayni signature ile mevcut olmasi
...
class TestStaticParity:
    ...
    def test_same_top_level_functions(self) -> None:
        ...
        legacy_funcs = _collect_top_level_defs(LEGACY_DIR / "export_results.py")
        new_funcs = _collect_top_level_defs(SCRIPTS_DIR / "export_results.py")

        for required in self.EXPORT_REQUIRED_FUNCS:
    ...
    def test_function_arg_counts_match(self) -> None:
        ...
        for fname in self.EXPORT_REQUIRED_FUNCS:
...
class TestJSONSchemaStability:
    ...
    EXPORT_TOP_LEVEL_KEYS = {
        "summary", "program_info", "functions", "strings",
        "call_graph", "decompilation",
    }
    EXPORT_SUMMARY_KEYS = {
        "program", "function_count", "string_count",
        "call_graph_nodes", "call_graph_edges",
        "decompiled_success", "decompiled_failed",
    }
    EXPORT_PROGRAM_INFO_KEYS = {
        "name", "path", "language", "processor", "endian",
        "address_size", "compiler", "executable_format",
        "image_base", "min_address", "max_address",
        "entry_points", "memory_blocks",
    }
    EXPORT_MEMORY_BLOCK_KEYS = {
        "name", "start", "end", "size", "permissions", "type",
    }
    ...
    def test_migrated_has_all_top_level_keys(self) -> None:
        keys = self._extract_dict_keys_from_source(SCRIPTS_DIR / "export_results.py")
        missing = self.EXPORT_TOP_LEVEL_KEYS - keys
        assert not missing, f"Top-level JSON anahtarlari eksik: {missing}"

    def test_migrated_summary_keys_preserved(self) -> None:
        keys = self._extract_dict_keys_from_source(SCRIPTS_DIR / "export_results.py")
        missing = self.EXPORT_SUMMARY_KEYS - keys
        assert not missing, f"Summary dict anahtarlari eksik: {missing}"

    def test_migrated_program_info_keys_preserved(self) -> None:
        keys = self._extract_dict_keys_from_source(SCRIPTS_DIR / "export_results.py")
        missing = self.EXPORT_PROGRAM_INFO_KEYS - keys
        assert not missing, f"Program info dict anahtarlari eksik: {missing}"

    def test_migrated_memory_block_keys_preserved(self) -> None:
        keys = self._extract_dict_keys_from_source(SCRIPTS_DIR / "export_results.py")
        missing = self.EXPORT_MEMORY_BLOCK_KEYS - keys
        assert not missing, f"Memory block dict anahtarlari eksik: {missing}"
...
class TestMigrationHeaders:
    ...
    def test_header_updated(self) -> None:
        script = SCRIPTS_DIR / "export_results.py"
    ...
    def test_legacy_retains_jython_header(self) -> None:
        ...
        legacy = LEGACY_DIR / "export_results.py"
