...
``monkeypatch`` ile resolve_tool/_run_tool sahtelenerek test edilir.
...
def _fake_completed(stdout: str, returncode: int = 0, stderr: str = "") -> pd._ChildRun:
    """Sahte araç sonucu (``_run_capped`` biçimi; çıktı bayt)."""
    return pd._ChildRun(returncode, stdout.encode("utf-8"), stderr.encode("utf-8"))
...
class TestDecompilePycdc:
    def test_pycdc_basari(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(pd, "_run_tool", lambda *a, **k: _fake_completed(real_source))
    ...
    def test_pycdc_bos_cikti_reddedilir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(pd, "_run_tool", fake_run)
    ...
    def test_pycdc_returncode_hata_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(pd, "_run_tool",
    ...
    def test_pycdc_timeout_fallback(self, tmp_path, monkeypatch):
        monkeypatch.setattr(pd, "resolve_tool",
                            lambda name, **k: "/fake/pycdc" if name == "pycdc" else None)
        # Sözleşme: zaman aşımında (bekçi öldürdü) gelen çıktı -- olsa bile -- kullanılmaz.
        monkeypatch.setattr(pd, "_run_tool",
                            lambda *a, **k: pd._ChildRun(None, b"x = 1\n", b"", "timeout"))
        pyc = tmp_path / "t.pyc"
        pyc.write_bytes(repair_pyc_header(_marshal_body(), RUNNING))
...
def _pycdc_only(monkeypatch, stdout, returncode: int = 0, stderr: str = "") -> None:
    """Yalnız pycdc 'kurulu' (pycdas yok); _run_tool sabit pycdc sonucu döner."""
    monkeypatch.setattr(pd, "resolve_tool",
                        lambda name, **k: "/fake/pycdc" if name == "pycdc" else None)
    monkeypatch.setattr(pd, "_run_tool",
...
class TestPycdcCiktiKodlama:
    def test_gecersiz_utf8_cikti_kaybolmaz(self, tmp_path, monkeypatch):
        ...
        Araç çağrısı ham bayt döndürür (``_run_capped``); çözme bu modülde, hatalı bayt
        U+FFFD olur.
        """
        raw = "A = 'dünya'\nB = '".encode("utf-8") + b"\xed\xb3\xbf" + b"'\n"

        def fake_run(cmd, **k):
            assert "text" not in k      # metin kipi yok: çözme çağıranda
            return pd._ChildRun(0, raw, b"")
...
class TestPycdasAracYolu:
    def test_pycdas_extra_paths_ile_bulunur(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(pd, "resolve_tool", fake_resolve)
        monkeypatch.setattr(pd, "_run_tool", lambda cmd, **k: _fake_completed(
            "m.pyc (Python 3.8)\n[Code]\n    File Name: m.py\n"))
        pyc = tmp_path / "m.pyc"
        pyc.write_bytes(repair_pyc_header(_marshal_body(), OTHER))
